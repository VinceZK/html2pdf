...
/*! html2pdf-v0.1.0@2015/09/04 05:09:20 */
...
$scope.jobList[currIndex].STATUS_TEXT=snapshotSer.getStatusText(job.STATUS),job.STATUS<3)allFinished=!1;else if(3===job.STATUS){
var downloadLink=1===job.NUM_PAGES?ossHost+job.JOB_GUID+"."+eval("("+job.OPTIONS+")").format:ossHost+job.JOB_GUID+".zip";angular.element(jobListTableRowEle[currIndex+1].children[0]).replaceWith('<td><a href="'+downloadLink+'" target="_blank">'+job.JOB_GUID+"</a></td>"),
...
$scope.inputURL=null,$scope.jobList=[],$scope.phantomOpts={format:"pdf",width:$window.innerWidth,height:$window.innerHeight,
orientation:"Portrait",paperFormat:"A4",margin:"1cm",headerAndFooter:!0},$scope.margin="Default",$scope.customMargin=.5,$scope.subemail=null,
$scope.switchUrlInput=function(){"single"===$scope.urlInput?switch2MultiUrlInput():switch2SingleUrlInput()},$scope.toggleAdvancedOptions=function(){
"show"===$scope.advancedOption?hideAdvancedOptions():showAdvancedOptions()},$scope.submitSnapshotJob=function(){var a=[],b=!1;
if(alertSer.clearAlerts(),"single"===$scope.urlInput?snapshotSer.checkURLisValid($scope.inputURL)?a.push($scope.inputURL):(alertSer.addAlert("danger","输入的网址不合法，也许你忘记加上“http://”或者“https://”了！"),
...
}"pdf"===$scope.phantomOpts.format&&($scope.phantomOpts.height=1e5),snapshotSer.submitSnapshotJob(a,$scope.phantomOpts,function(b,c){
b?alertSer.addAlert("danger",b):c.errorMsg?alertSer.addAlert("danger",c.errorMsg):(alertSer.addAlert("success",c.successMsg),
...
alertSer.addAlert("success",c.successMsg))})})},switch2SingleUrlInput(),hideAdvancedOptions()}]).controller("addEmailCtrl",["$scope","$modalInstance",function(a,b){
a.subemail=null,a.focusSubEmail=!0,a.ok=function(){a.subemail&&b.close(a.subemail)},a.cancel=function(){b.dismiss("cancel");
}}]).factory("snapshotSer",["$http",function(a){function b(b,c,d){a.post("/pub/api/snapshot",{urls:b,opts:c}).success(function(a){
d(null,a)}).error(function(a){d(a)})}function c(b,c){a.get("/pub/api/snapshot",{params:{jobGuids:b}}).success(function(a){
c(a.jobList)}).error(function(){c(null)})}function d(a){var b="";switch(a){case 0:b="等待付款";break;case 1:b="就绪";break;case 2:
b="运行中";break;case 3:b="成功完成";break;case 4:b="运行失败"}return b}function e(b,c,d){a.put("/pub/api/snapshot",{jobGuids:b,email:c
}).success(function(a){d(null,a)}).error(function(a){d(a)})}function f(a){var b=a,c=/http(s)?:\/\/([\w-]+\.)*[\w-]+(:[0-9]+)*(\/[\w- .\/?%&=]*)?/,d=new RegExp(c);
return d.test(b)}function g(a){for(var b=a.split(/\n/),c=[],d=0;d<b.length;d++)/\S/.test(b[d])&&c.push(b[d].replace(/(^\s*)|(\s*$)/g,""));
return c}return{submitSnapshotJob:b,getJobList:c,getStatusText:d,addSubscribeEmail:e,textarea2Array:g,checkURLisValid:f}}]).directive("focusMe",["$timeout","$parse",function(a,b){
return{link:function(c,d,e){var f=b(e.focusMe);c.$watch(f,function(b){b===!0&&a(function(){d[0].focus()})}),d.bind("blur",function(){
