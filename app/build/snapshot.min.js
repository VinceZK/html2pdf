...
/*! html2pdf-v0.1.0@2015/08/25 07:08:11 */
"use strict";angular.module("snapshot",["ui.bootstrap","dk-alert"]).controller("snapshotCtrl",["$scope","$window","$timeout","$modal","snapshotSer","alertSer",function($scope,$window,$timeout,$modal,snapshotSer,alertSer){
function switch2SingleUrlInput(){$scope.urlInput="single",$scope.switchDesc="切换至批量转换"}function switch2MultiUrlInput(){$scope.urlInput="multi",
$scope.switchDesc="切换至单网址转换"}function showAdvancedOptions(){$scope.advancedOption="show",$scope.optionDesc="关闭高级选项"}function hideAdvancedOptions(){
$scope.advancedOption="hide",$scope.optionDesc="高级选项"}function refreshJobStatus(){if(0!==$scope.jobList.length){var jobGuids=[],allFinished=!0;
angular.forEach($scope.jobList,function(a){a.STATUS<3&&jobGuids.push(a.JOB_GUID)}),snapshotSer.getJobList(jobGuids,function(newJobList){
var jobListTableRowEle=angular.element("#jobListTable tr"),currIndex;angular.forEach(newJobList,function(job){if(angular.forEach($scope.jobList,function(a,b){
return a.JOB_GUID===job.JOB_GUID?void(currIndex=b):void 0}),$scope.jobList[currIndex].STATUS!==job.STATUS){if($scope.jobList[currIndex].STATUS=job.STATUS,
$scope.jobList[currIndex].STATUS_TEXT=snapshotSer.getStatusText(job.STATUS),job.STATUS<3)allFinished=!1;else if(3===job.STATUS)angular.element(jobListTableRowEle[currIndex+1].children[0]).replaceWith('<td><a href="'+ossHost+job.JOB_GUID+'.zip" target="_blank">'+job.JOB_GUID+"</a></td>"),
angular.element(jobListTableRowEle[currIndex+1].children[2]).addClass("snapshot-table-success");else if(4===job.STATUS){var errMsg=eval("("+job.PROCESS_MSG+")");
angular.element(jobListTableRowEle[currIndex+1].children[0]).replaceWith('<td><a tabindex="0" data-toggle="popover" data-trigger="focus" title="出错信息"data-content="'+errMsg.msgText.substr(0,42)+'">'+job.JOB_GUID+"</a></td>"),
angular.element('[data-toggle="popover"]').popover(),angular.element(jobListTableRowEle[currIndex+1].children[2]).addClass("snapshot-table-fail");
}}else job.STATUS<3&&(allFinished=!1)}),allFinished||$timeout(refreshJobStatus,1e3)})}}var ossHost="http://snapshots.darkhouse.com.cn/snapshots/";
$scope.inputURL=null,$scope.jobList=[],$scope.phantomOpts={format:"pdf",width:$window.innerWidth,height:1e5,orientation:"Portrait",
paperFormat:"A4",margin:"1cm",headerAndFooter:!0},$scope.margin="Default",$scope.customMargin=.5,$scope.subemail=null,$scope.switchUrlInput=function(){
"single"===$scope.urlInput?switch2MultiUrlInput():switch2SingleUrlInput()},$scope.toggleAdvancedOptions=function(){"show"===$scope.advancedOption?hideAdvancedOptions():showAdvancedOptions();
},$scope.submitSnapshotJob=function(){var a=[],b=!1;if(alertSer.clearAlerts(),"single"===$scope.urlInput?snapshotSer.checkURLisValid($scope.inputURL)?a.push($scope.inputURL):(alertSer.addAlert("danger","输入的网址不合法，也许你忘记加上“http://”或者“https://”了！"),
b=!0):(a=snapshotSer.textarea2Array($scope.inputURL),angular.forEach(a,function(a){snapshotSer.checkURLisValid(a)||(alertSer.addAlert("danger","输入的网址中存在不合法："+a),
b=!0)})),!b){switch($scope.margin){case"Default":$scope.phantomOpts.margin="1cm";break;case"None":$scope.phantomOpts.margin="0cm";
break;case"Minimum":$scope.phantomOpts.margin="0.3cm";break;case"Custom":angular.isNumber($scope.customMargin)&&$scope.customMargin>0?$scope.phantomOpts.margin=$scope.customMargin+"cm":$scope.phantomOpts.margin="1cm";
}snapshotSer.submitSnapshotJob(a,$scope.phantomOpts,function(b,c){b?alertSer.addAlert("danger",b):c.errorMsg?alertSer.addAlert("danger",c.errorMsg):(alertSer.addAlert("success",c.successMsg),
$scope.inputURL=null,$scope.jobList.push({JOB_GUID:c.jobGuid,STATUS:1,STATUS_TEXT:snapshotSer.getStatusText(1),JOB_DESC:a[0]
}),$scope.subemail&&snapshotSer.addSubscribeEmail([c.jobGuid],$scope.subemail),$timeout(refreshJobStatus,1e3))})}},$scope.enterToSubmit=function(a){
13===a.keyCode&&$scope.submitSnapshotJob()},$scope.registerEmail=function(a){var b=$modal.open({templateUrl:"addEmail.html",
controller:"addEmailCtrl",size:a});b.result.then(function(a){var b=[];angular.forEach($scope.jobList,function(a){b.push(a.JOB_GUID);
}),snapshotSer.addSubscribeEmail(b,a,function(b,c){b?alertSer.addAlert("danger",b):c.errorMsg?alertSer.addAlert("danger",c.errorMsg):($scope.subemail=a,
alertSer.addAlert("success",c.successMsg))})})},switch2SingleUrlInput(),hideAdvancedOptions(),snapshotSer.getLatestPageList(60,function(a){
$scope.latestPages=a})}]).controller("addEmailCtrl",["$scope","$modalInstance",function(a,b){a.subemail=null,a.focusSubEmail=!0,
a.ok=function(){a.subemail&&b.close(a.subemail)},a.cancel=function(){b.dismiss("cancel")}}]).factory("snapshotSer",["$http",function(a){
function b(b,c,d){a.post("/pub/api/snapshot",{urls:b,opts:c}).success(function(a){d(null,a)}).error(function(a){d(a)})}function c(b,c){
a.get("/pub/api/snapshot",{params:{jobGuids:b}}).success(function(a){c(a.jobList)}).error(function(){c(null)})}function d(b,c){
a.get("/pub/api/snapshot/latest",{params:{num:b}}).success(function(a){c(a)}).error(function(){c(null)})}function e(a){var b="";
switch(a){case 0:b="等待付款";break;case 1:b="就绪";break;case 2:b="运行中";break;case 3:b="成功完成";break;case 4:b="运行失败"}return b}function f(b,c,d){
a.put("/pub/api/snapshot",{jobGuids:b,email:c}).success(function(a){d(null,a)}).error(function(a){d(a)})}function g(a){var b=a,c=/http(s)?:\/\/([\w-]+\.)*[\w-]+(:[0-9]+)*(\/[\w- .\/?%&=]*)?/,d=new RegExp(c);
return d.test(b)}function h(a){for(var b=a.split(/\n/),c=[],d=0;d<b.length;d++)/\S/.test(b[d])&&c.push(b[d].replace(/(^\s*)|(\s*$)/g,""));
return c}return{submitSnapshotJob:b,getJobList:c,getStatusText:e,addSubscribeEmail:f,getLatestPageList:d,textarea2Array:h,
checkURLisValid:g}}]).directive("focusMe",["$timeout","$parse",function(a,b){return{link:function(c,d,e){var f=b(e.focusMe);
c.$watch(f,function(b){b===!0&&a(function(){d[0].focus()})}),d.bind("blur",function(){c.$apply(f.assign(c,!1))})}}}]);
/*! html2pdf-v0.1.0@2015/08/25 07:08:11 */
angular.module("dk-alert",[]).controller("alertCtrl",["$rootScope","$scope",function(a,b){a.alerts=[],b.closeAlert=function(b){
a.alerts.splice(b,1)}}]).factory("alertSer",["$rootScope",function(a){function b(b,c){a.alerts.push({type:b,msg:c})}function c(){
a.alerts=[]}return{addAlert:b,clearAlerts:c}}]);