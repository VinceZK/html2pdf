...
var loop = function() {
  ...
  if(line.headerAndFooter){
      //page.paperSize = line.paperSize ||
    page.paperSize =
      {
          //format: line.paperFormat || 'A4', "A4 format seems not work in phantomjs 2.1
          width: line.width || 1280,
          height: line.height || 960,
          ...
          header: {
              ...
              contents: phantom.callback(function (pageNum, numPages) {
                  ...
                  var currentDateStr = (currentDate.getMonth()+1) + '/' + currentDate.getDate() +
                      '/'+ currentDate.getFullYear();
                  return "<p style='font-size:12px;text-align:center'>" +
                      "html2pdf.cn <span style='float:left'>"+ currentDateStr + "</span></p><hr color='#eee' size='1'>";
              })
          },
          footer: {
              height: "1cm",
              contents: phantom.callback(function (pageNum, numPages) {
                  return "<hr color='#eee' size='1'><p style='font-size:12px'>"
                      + page.url + "<span style='float:right'>" + pageNum + " / " + numPages + "</span></p></div>";
              })
          }
      };
  }else{
  ...
  if (line.crop) {
    page.clipRect = {
      width: line.crop.width || line.width,
      height: line.crop.height || line.height,
      top: line.crop.top || 0,
  ...
  page.open(line.url, function(requestStatus) {
    ...
    var render = function() {
      setTimeout(function() {
        ...
        if(line.pageInfo === true){
          line.pageInfo = {};
          line.pageInfo.title = page.evaluate(function(){
              return document.title;
          })||'';

          line.pageInfo.keywords = page.evaluate(function(){
              return document.querySelector('meta[name="keywords"]').getAttribute('content');
          })||'';

          line.pageInfo.description = page.evaluate(function(){
              return document.querySelector('meta[name="description"]').getAttribute('content');
          })||'';
        }

        page.paperSize.height = 2000;
